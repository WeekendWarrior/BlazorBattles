...
namespace BlazorBattles.Server.Controllers
{
    ...
    public class BattleController : ControllerBase
    {
        ...
        public async Task<IActionResult> StartBattle([FromBody] int opponentId)
        {
            ...
            if (opponent == null || opponent.IsDeleted) return NotFound($"opponentId {opponentId} not available.");

            if (attacker.Id == opponent.Id) return BadRequest($"The opponentId {opponentId} cannot be the same as the logged in user (attacker) Id!");
        ...
        public async Task<IActionResult> GetHistory()
        {
            ...
            var sortedHistory = history.OrderByDescending(h => h.BattleDate);
